...
export function getFieldTypeFromValues(values: JSONValue[]): InferenceResult {
    ...
    const nonNull = values.filter((v): v is Exclude<JSONValue, null> => v !== null);
    ...
    if (nonNull.length > 0 && nonNull.every(v => typeof v === 'string' && isDateString(v))) {
    ...
    if (numericCandidates) {
        const nums = nonNull.map(v => typeof v === 'number' ? v : Number((v as string).trim()));
    ...
    const normalize = (v: JSONValue) => v === null ? null : String(v).trim().toLowerCase();
